...
def lcd_set_page(page, column):
   ...
   # msb = msb | 0x10                  
   # page = page | 0xb0                               
   lcd_tranfer_data(page | 0xb0,0)
   lcd_tranfer_data((column >> 4) | 0x10,0)
   lcd_tranfer_data(column & 0x0f,0)
...
def lcd_byte(bits):
   ...
   for i in range(0, 8): 
      ...
      if(tmp & 0x80): # grab the most sig bit???
         GPIO.output(LCD_SI, True)
      else:
         GPIO.output(LCD_SI, False)
      tmp = (tmp<<1) # shift bits
      GPIO.output(LCD_CLK, True) # data is read on the rising edge of clock
