...
class Client(threading.Thread):
    ...
    def __init__(self, graf_props):
        ...
        # draw an initial frame, synchronously, to both buffers
        try:
            ...
            self.graceful_fail("draw_frame failed in __init__")
    ...
    def graceful_fail(self, msg=None):
        try: # ie. have been initialised
            self.width = self.height = 64
            self.buffer_lock = threading.Lock()
            self.buffer_a = pygame.Surface((self.width, self.height), depth=32)
            ...
            font = pygame.freetype.Font("chicago.bdf")
            font.render_to(self.buffer_a, (20,20), "Fale!", (255,255,255), (0,0,0))
            parent_server.remove_client(self)
        except:
            "oops"
        
        if msg:
            print("Fale: %s. %s" % (self.__class__.__name__, msg))
        else:
            print("False: %s" % (self.__class__.__name__))
